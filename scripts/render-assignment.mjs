...
function variableMap(problem, currentValues) {
  ...
  const selectorStackLb = numericValue(values, "selector_W_lb");
  const selectorDiameter = numericValue(values, "selector_d_mm");
  const selectorYieldStrength = numericValue(values, "selector_S_y_MPa");
  const selectorRequiredFos = numericValue(values, "selector_FS_req");
  const selectorGravity = numericValue(values, "selector_g");

  if (
    Number.isFinite(selectorStackLb) && selectorStackLb > 0 &&
    Number.isFinite(selectorDiameter) && selectorDiameter > 0 &&
    Number.isFinite(selectorYieldStrength) && selectorYieldStrength > 0 &&
    Number.isFinite(selectorRequiredFos) && selectorRequiredFos > 0 &&
    Number.isFinite(selectorGravity) && selectorGravity > 0
  ) {
    const massKg = selectorStackLb * 0.45359237;
    const loadN = massKg * selectorGravity;
    const forcePerPlane = loadN / 2;
    const area = Math.PI * selectorDiameter ** 2 / 4;
    const tauActual = forcePerPlane / area;
    const tauYield = selectorYieldStrength / Math.sqrt(3);
    const tauAllow = tauYield / selectorRequiredFos;
    const actualFos = tauYield / tauActual;
    const minimumDiameter = Math.sqrt(2 * loadN / (Math.PI * tauAllow));
    const passes = actualFos >= selectorRequiredFos;
    values.selector_mass_kg = formatDerived(massKg, 2);
    values.selector_load_N = formatDerived(loadN, 1);
    values.selector_force_per_plane_N = formatDerived(forcePerPlane, 1);
    values.selector_area_mm2 = formatDerived(area, 2);
    values.selector_tau_actual_MPa = formatDerived(tauActual, 2);
    values.selector_tau_y_MPa = formatDerived(tauYield, 2);
    values.selector_tau_allow_MPa = formatDerived(tauAllow, 2);
    values.selector_fos_actual = formatDerived(actualFos, 2);
    values.selector_d_min_mm = formatDerived(minimumDiameter, 2);
    values.selector_diameter_assessment = selectorDiameter >= minimumDiameter
      ? "The specified pin diameter meets the calculated minimum for this simplified double-shear check."
      : "The specified pin diameter is smaller than the calculated minimum for this simplified double-shear check.";
    values.selector_fos_assessment = passes
      ? "The actual factor of safety meets or exceeds the requirement."
      : "The actual factor of safety is below the requirement.";
    values.selector_engineering_assessment = passes
      ? "The specified pin satisfies the assigned average double-shear yielding check."
      : "The specified pin does not satisfy the assigned average double-shear yielding check.";
  }
  ...
  const hitchHorizontalLoadKn = numericValue(values, "hitch_P_kN");
  const hitchVerticalLoadKn = numericValue(values, "hitch_W_kN");
  const hitchHorizontalOffsetMm = numericValue(values, "hitch_L_mm");
  const hitchVerticalOffsetMm = numericValue(values, "hitch_h_mm");
  const hitchDiameterMm = numericValue(values, "hitch_d_mm");
  const hitchYieldStrengthMpa = numericValue(values, "hitch_S_y_MPa");
  const hitchRequiredFos = numericValue(values, "hitch_n_req");

  if (
    Number.isFinite(hitchHorizontalLoadKn) && hitchHorizontalLoadKn > 0 &&
    Number.isFinite(hitchVerticalLoadKn) && hitchVerticalLoadKn > 0 &&
    Number.isFinite(hitchHorizontalOffsetMm) && hitchHorizontalOffsetMm >= 0 &&
    Number.isFinite(hitchVerticalOffsetMm) && hitchVerticalOffsetMm >= 0 &&
    Number.isFinite(hitchDiameterMm) && hitchDiameterMm > 0 &&
    Number.isFinite(hitchYieldStrengthMpa) && hitchYieldStrengthMpa > 0 &&
    Number.isFinite(hitchRequiredFos) && hitchRequiredFos > 0
  ) {
    const horizontalLoadN = hitchHorizontalLoadKn * 1000;
    const verticalLoadN = hitchVerticalLoadKn * 1000;
    const momentNmm = horizontalLoadN * hitchVerticalOffsetMm + verticalLoadN * hitchHorizontalOffsetMm;
    const areaMm2 = Math.PI * hitchDiameterMm ** 2 / 4;
    const inertiaMm4 = Math.PI * hitchDiameterMm ** 4 / 64;
    const outerRadiusMm = hitchDiameterMm / 2;
    const axialStressMpa = horizontalLoadN / areaMm2;
    const bendingStressMpa = momentNmm * outerRadiusMm / inertiaMm4;
    const maximumNormalStressMpa = axialStressMpa + bendingStressMpa;
    const maximumTransverseShearMpa = 4 * verticalLoadN / (3 * areaMm2);
    const allowableNormalStressMpa = hitchYieldStrengthMpa / hitchRequiredFos;
    const actualFos = hitchYieldStrengthMpa / maximumNormalStressMpa;
    const stressAtDiameter = (diameter) =>
      4 * horizontalLoadN / (Math.PI * diameter ** 2) +
      32 * momentNmm / (Math.PI * diameter ** 3);
    let lowerDiameter = Number.EPSILON;
    let upperDiameter = Math.max(hitchDiameterMm, 1);
    while (stressAtDiameter(upperDiameter) > allowableNormalStressMpa) {
      upperDiameter *= 2;
    }
    for (let iteration = 0; iteration < 100; iteration += 1) {
      const trialDiameter = (lowerDiameter + upperDiameter) / 2;
      if (stressAtDiameter(trialDiameter) > allowableNormalStressMpa) {
        lowerDiameter = trialDiameter;
      } else {
        upperDiameter = trialDiameter;
      }
    }
    const minimumDiameterMm = (lowerDiameter + upperDiameter) / 2;
    const recommendedDiameterMm = Math.ceil(minimumDiameterMm - 1e-12);
    const passes = maximumNormalStressMpa <= allowableNormalStressMpa * (1 + 1e-10);

    values.hitch_P_N = formatDerived(horizontalLoadN, 0);
    values.hitch_W_N = formatDerived(verticalLoadN, 0);
    values.hitch_M_Nmm = formatDerived(momentNmm, 0);
    values.hitch_M_kNm = formatDerived(momentNmm / 1e6, 2);
    values.hitch_area_mm2 = formatDerived(areaMm2, 1);
    values.hitch_I_mm4 = formatDerived(inertiaMm4, 1);
    values.hitch_c_mm = formatDerived(outerRadiusMm, 1);
    values.hitch_sigma_axial_MPa = formatDerived(axialStressMpa, 2);
    values.hitch_sigma_bending_MPa = formatDerived(bendingStressMpa, 2);
    values.hitch_sigma_max_MPa = formatDerived(maximumNormalStressMpa, 2);
    values.hitch_tau_max_MPa = formatDerived(maximumTransverseShearMpa, 2);
    values.hitch_sigma_allow_MPa = formatDerived(allowableNormalStressMpa, 2);
    values.hitch_fos_actual = formatDerived(actualFos, 2);
    values.hitch_d_min_mm = formatDerived(minimumDiameterMm, 2);
    values.hitch_d_recommended_mm = formatDerived(recommendedDiameterMm, 0);
    values.hitch_strength_assessment = passes
      ? "The assigned equivalent section meets the required first-yield factor of safety."
      : "The assigned equivalent section does not meet the required first-yield factor of safety.";
    values.hitch_diameter_assessment = hitchDiameterMm >= minimumDiameterMm
      ? "The assigned equivalent diameter meets or exceeds the calculated minimum."
      : "The assigned equivalent diameter is smaller than the calculated minimum.";
    values.hitch_engineering_assessment = passes
      ? `The ${formatDerived(hitchDiameterMm, 1)} mm equivalent section provides FS = ${formatDerived(actualFos, 2)}, meeting the required ${formatDerived(hitchRequiredFos, 1)} first-yield factor of safety for the prescribed static model.`
  ...
  const millDiameterMm = numericValue(values, "mill_D_mm");
  const millOverhangMm = numericValue(values, "mill_L_mm");
  const millForceN = numericValue(values, "mill_F_N");
  const millTangentialForceN = numericValue(values, "mill_Ft_N");
  const millYieldStrengthMpa = numericValue(values, "mill_Sy_MPa");
  const millRequiredFos = numericValue(values, "mill_n_req");

  if (
    Number.isFinite(millDiameterMm) && millDiameterMm > 0 &&
    Number.isFinite(millOverhangMm) && millOverhangMm > 0 &&
    Number.isFinite(millForceN) && millForceN >= 0 &&
    Number.isFinite(millTangentialForceN) && millTangentialForceN >= 0 &&
    Number.isFinite(millYieldStrengthMpa) && millYieldStrengthMpa > 0 &&
    Number.isFinite(millRequiredFos) && millRequiredFos > 0
  ) {
    const inertiaMm4 = Math.PI * millDiameterMm ** 4 / 64;
    const polarInertiaMm4 = Math.PI * millDiameterMm ** 4 / 32;
    const momentNmm = millForceN * millOverhangMm;
    const torqueNmm = millTangentialForceN * millDiameterMm / 2;
    const bendingStressMpa = momentNmm * (millDiameterMm / 2) / inertiaMm4;
    const torsionalStressMpa = torqueNmm * (millDiameterMm / 2) / polarInertiaMm4;
    const vonMisesStressMpa = Math.sqrt(bendingStressMpa ** 2 + 3 * torsionalStressMpa ** 2);
    ...
    values.mill_I_mm4 = formatDerived(inertiaMm4, 2);
    values.mill_J_mm4 = formatDerived(polarInertiaMm4, 2);
    values.mill_M_Nmm = formatDerived(momentNmm, 1);
    values.mill_T_Nmm = formatDerived(torqueNmm, 1);
    values.mill_sigma_b_MPa = formatDerived(bendingStressMpa, 3);
    values.mill_tau_t_MPa = formatDerived(torsionalStressMpa, 3);
    values.mill_sigma_vm_MPa = formatDerived(vonMisesStressMpa, 3);
    values.mill_fos = Number.isFinite(actualFos) ? formatDerived(actualFos, 1) : "unbounded";
    values.mill_sigma_allow_MPa = formatDerived(allowableStressMpa, 1);
    values.mill_force_at_required_fos_N = formatDerived(forceAtRequiredFosN, 1);
  ...
  const vBeltTensionRatio = numericValue(values, "v_belt_k");
  const vBeltEquivalentAreaMm2 = numericValue(values, "v_belt_A_eq_mm2");
  const vBeltAllowableStressMpa = numericValue(values, "v_belt_sigma_allow_MPa");

  if (
    Number.isFinite(vBeltPowerKw) && vBeltPowerKw > 0 &&
    Number.isFinite(vBeltSpeedRpm) && vBeltSpeedRpm > 0 &&
    Number.isFinite(vBeltPulleyDiameterMm) && vBeltPulleyDiameterMm > 0 &&
    Number.isFinite(vBeltTensionRatio) && vBeltTensionRatio >= 0 && vBeltTensionRatio < 1 &&
    Number.isFinite(vBeltEquivalentAreaMm2) && vBeltEquivalentAreaMm2 > 0 &&
    Number.isFinite(vBeltAllowableStressMpa) && vBeltAllowableStressMpa > 0
  ) {
    const powerW = vBeltPowerKw * 1000;
    const omegaRadS = 2 * Math.PI * vBeltSpeedRpm / 60;
    const torqueNm = powerW / omegaRadS;
    const radiusM = vBeltPulleyDiameterMm / 2000;
    const tensionDifferenceN = torqueNm / radiusM;
    const tightTensionN = tensionDifferenceN / (1 - vBeltTensionRatio);
    const slackTensionN = vBeltTensionRatio * tightTensionN;
    const beltSpeedMS = omegaRadS * radiusM;
    const powerCheckKw = tensionDifferenceN * beltSpeedMS / 1000;
    const tightStressMpa = tightTensionN / vBeltEquivalentAreaMm2;
    const slackStressMpa = slackTensionN / vBeltEquivalentAreaMm2;
    const allowableRatio = vBeltAllowableStressMpa / tightStressMpa;
    const passes = tightStressMpa <= vBeltAllowableStressMpa * (1 + 1e-10);

    values.v_belt_power_W = formatDerived(powerW, 0);
    values.v_belt_omega_rad_s = formatDerived(omegaRadS, 2);
    values.v_belt_torque_Nm = formatDerived(torqueNm, 2);
    values.v_belt_radius_m = formatDerived(radiusM, 3);
    values.v_belt_tension_difference_N = formatDerived(tensionDifferenceN, 0);
    values.v_belt_tension_difference_kN = formatDerived(tensionDifferenceN / 1000, 3);
    values.v_belt_T1_N = formatDerived(tightTensionN, 0);
    values.v_belt_T1_kN = formatDerived(tightTensionN / 1000, 3);
    values.v_belt_T2_N = formatDerived(slackTensionN, 0);
    values.v_belt_T2_kN = formatDerived(slackTensionN / 1000, 3);
    values.v_belt_speed_m_s = formatDerived(beltSpeedMS, 3);
    values.v_belt_power_check_kW = formatDerived(powerCheckKw, 2);
    values.v_belt_sigma1_MPa = formatDerived(tightStressMpa, 2);
    values.v_belt_sigma2_MPa = formatDerived(slackStressMpa, 2);
    values.v_belt_allowable_ratio = formatDerived(allowableRatio, 2);
    values.v_belt_stress_assessment = passes
      ? "The instructor-defined equivalent-tension model passes the assigned average tensile-stress criterion; the tight side governs."
      : "The instructor-defined equivalent-tension model does not pass the assigned average tensile-stress criterion; the tight side governs.";
    values.v_belt_engineering_recommendation = passes
      ? `The idealized equivalent-area model is acceptable for the assigned load: the governing tight-side stress is ${formatDerived(tightStressMpa, 2)} MPa, below the ${formatDerived(vBeltAllowableStressMpa, 2)} MPa instructor-specified allowable stress.`
      : `The idealized equivalent-area model is not acceptable for the assigned load: the governing tight-side stress is ${formatDerived(tightStressMpa, 2)} MPa, above the ${formatDerived(vBeltAllowableStressMpa, 2)} MPa instructor-specified allowable stress. Increase the effective area, use an additional belt, or reduce transmitted power.`;
  }
...
function qmdDocument(problem, variant, type, outDir) {
  ...
  const renderQuestion = (question) => {
    ...
    return `### ${question.displayNumber || numberById.get(question.id)}. ${question.title}
