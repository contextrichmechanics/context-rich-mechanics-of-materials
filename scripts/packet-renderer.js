(function () {
  ...
  function variableMap(problem, overrides = {}) {
    ...
    if (
      ...
      const minimumHeight = Math.cbrt(
        4 * loadN * hoopLength ** 3 /
      ...
      const revisedDeflection = loadN * hoopLength ** 3 /
        (3 * elasticModulusMpa * revisedInertia);
      const strengthPasses = factorOfSafety >= hoopRequiredFos;
      const deflectionPasses = tipDeflection <= hoopAllowableDeflection;
      const revisedPasses = revisedFactorOfSafety >= hoopRequiredFos &&
        revisedDeflection <= hoopAllowableDeflection;
