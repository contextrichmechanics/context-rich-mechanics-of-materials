...
function selectedQuestions(problem, type) {
  ...
  return problem.questions.filter((question) => {
    ...
    const selected = checkbox ? checkbox.checked : question.selected;
    return selected && question[audienceKey] !== false;
  });
}
...
function renderDocumentBody(type, embeddedImages = {}) {
  ...
  const questions = selectedQuestions(problem, type);
  ...
  return `
    <article class="generated-packet">
      ...
      ${showIdealizedImage ? renderIdealizedImage(problem, embeddedImages) : ""}
...
async function embeddedPacketImages(problem, questions, type) {
  const isInstructor = type === "instructor";
  const showIdealizedImage =
    isInstructor ||
    problem.showIdealizedImageInStudentPacket === true;
  const sources = [
    ...
    showIdealizedImage ? packetImageUrl(problem, problem.idealizedImage) : "",
...
async function fullHtmlDocument(type) {
  const problem = selectedProblem();
  const questions = selectedQuestions(problem, type);
  const embeddedImages = await embeddedPacketImages(problem, questions, type);
