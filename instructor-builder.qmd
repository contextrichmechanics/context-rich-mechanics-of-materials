...
function imagePath(problem, embeddedImage) {
  if (embeddedImage) {
    return embeddedImage;
  }
  return new URL(problem.image, window.location.href).href;
}
...
function renderIdealizedImage(problem) {
  ...
  return `
      <section>
        ...
        <figure>
          <img src="${new URL(problem.idealizedImage, window.location.href).href}" alt="${escapeHtml(alt)}">
        </figure>
      </section>
  `;
}
...
function renderQuestionSection(problem, questions, section, isInstructor, numberById) {
...
function renderDocumentBody(type, embeddedImage = "") {
  const problem = selectedProblem();
  ...
  return `
    <article class="generated-packet">
      ...
      <figure>
        <img src="${imagePath(problem, embeddedImage)}" alt="${escapeHtml(problem.title)} problem image">
      </figure>
      ...
      ${renderQuestionSection(problem, questions, questionSections[0], isInstructor, numberById)}

      ${renderQuestionSection(problem, questions, questionSections[1], isInstructor, numberById)}
      ...
      ${renderQuestionSection(problem, questions, questionSections[2], isInstructor, numberById)}
    </article>
  `;
}
...
async function embeddedImageData(problem) {
  try {
    const response = await fetch(problem.image);
...
async function fullHtmlDocument(type) {
  const problem = selectedProblem();
  const embeddedImage = await embeddedImageData(problem);
...
<body>
${renderDocumentBody(type, embeddedImage)}
</body>
</html>`;
}
